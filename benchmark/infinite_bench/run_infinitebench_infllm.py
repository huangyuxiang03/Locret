...
def get_pred(
    ...
    """
    input_tokens = truncate_by_tokens(input_text.replace("$$MASK$$", "**MASK**"), tok, max_input_length)
    if verbose:
    ...
    if attn_type == "vllm":
        ...
        searcher = GreedySearch(model, tok)
        extra_end_token_ids = []
        extra_end_token_ids.append(tok.encode("<|eot_id|>", add_special_tokens=False)[0])
        outputs = searcher.generate(input_ids=input_tensors['input_ids'], max_length=generation_config.max_new_tokens, chunk_size=8192, extra_end_token_ids=extra_end_token_ids)
        searcher.clear()
...
def load_model(
    ...
    # inf_config = OmegaConf.load("/home/test/test01/hyx/MInference/experiments/infinite_bench/inf-llm_phi-3.yaml").model
    inf_config = OmegaConf.load("/home/test/test01/hyx/InfLLM-main/config/llama-3-inf-llm.yaml").model
    llm = patch_hf(llm, inf_config.type, **inf_config)
