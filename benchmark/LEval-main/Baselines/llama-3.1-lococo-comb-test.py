...
from llama.modeling_llama_comb import LlamaForCausalLM
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', choices=['hh', 'conv'], default='conv')
    # parser.add_argument('--mem_size', type=int, default=512)
    parser.add_argument('--mem_size', type=int, default=16384)
    parser.add_argument('--local_len', type=int, default=0)
    # parser.add_argument('--hh_keep_rate', type=float, default=0.5)
    parser.add_argument('--hh_keep_rate', type=float, default=0.875)
    ...
    # model_path = f"/home/test/test01/hyx/LoCoCo-main/output_llama-3-no-lora"
    model_path = f"/home/test/test01/hyx/LoCoCo-main/output_llama-3-long"
    model_path_ori = f"/home/test/test01/hyx/Llama-3.1-8B-Instruct"
    open_source_model = f"Llama-3.1-8B-Instruct-lococo-2048"
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path_ori, trust_remote_code=True)
    # breakpoint()
    ...
    model = LlamaForCausalLM.from_pretrained(model_path, device_map='cuda', attn_implementation="eager", torch_dtype=torch.bfloat16)
