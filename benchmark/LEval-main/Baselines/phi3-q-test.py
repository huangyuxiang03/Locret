...
def gen(model, input_ids, max_new_tokens, eos_token_id):
    ...
    for b in range(0, input_ids.shape[-1], BS):
        ...
        attention_mask = torch.ones_like(input_ids[:, :e])
        output = model(input_ids[:, b:e], attention_mask=attention_mask, past_key_values=past_key_values, position_ids=position_ids, cache_position=cache_position)
    ...
    for _ in range(max_new_tokens-1):
        ...
        attention_mask = torch.ones((1, cache_position[-1] + 1), device=input_id.device, dtype=input_id.dtype)
        output = model(input_id, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, cache_position=cache_position)
        past_key_values = output.past_key_values
        input_id = output.logits[:, -1, :].argmax(dim=-1, keepdim=True)
    ...
    for file_name in key_data_pairs:
        ...
        for d in tqdm(data):
            ...
            for inst, out in zip(instructions, outputs):
                ...
                sample = gen(model, inputs.input_ids, max_new_tokens, 32007)
                # sample = model.generate(**inputs, do_sample=False, max_new_tokens=max_new_tokens, cache_implementation="quantized", cache_config={"backend": "quanto", "nbits": 2})
...
if __name__ == "__main__":
    ...
    open_source_model = f"Phi3-mini-128K-q-2bits"
    ...
    # breakpoint()
    model = Phi3ForCausalLM.from_pretrained(model_path, trust_remote_code=True, torch_dtype=torch.bfloat16, device_map='auto', attn_implementation="eager")
    model = model.eval()
